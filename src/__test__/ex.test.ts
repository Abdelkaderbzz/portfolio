...
describe('settingsSlice', () => {
  it('should toggle the value of isSidebarOpened when toggleSidebar is called', () => {
    // Arrange
    const initialState: settingsState = {
      isSidebarOpened: true,
      isNavigationbarOpened: true,
    };
    const action = settingsSlice.actions.toggleSidebar();

    // Act
    const newState = settingsSlice.reducer(initialState, action);

    // Assert
    expect(newState.isSidebarOpened).toBe(false);
  });

  it('should set isSidebarOpened to true when openSidebar is called', () => {
    // Arrange
    const initialState: settingsState = {
      isSidebarOpened: false,
      isNavigationbarOpened: true,
    };
    const action = settingsSlice.actions.openSidebar();

    // Act
    const newState = settingsSlice.reducer(initialState, action);

    // Assert
    expect(newState.isSidebarOpened).toBe(true);
  });

  it('should set isSidebarOpened to false when closeSidebar is called', () => {
    // Arrange
    const initialState: settingsState = {
      isSidebarOpened: true,
      isNavigationbarOpened: true,
    };
    const action = settingsSlice.actions.closeSidebar();
